<template>
  <v-container id="department-list" fluid tag="section">
            ...
            >
              <template slot="body">
                <tr v-for="(item, index) in departments" :key="index">
                  ...
                  <td>{{ item.unit ? item.unit.unit_name : "N/A" }}</td>
                  <td>{{ item.status }}</td>
  ...
  methods: {
    setHeaders() {
      this.headers = [
        ...
        {
          text: this.$t("department.unit"),
          align: "left",
          sortable: true,
          value: "unit.unit_name",
        },
        {
          text: this.$t("department.status"),
          align: "left",
          sortable: false,
          value: "status",
        },
      
        { text: this.$t("department.action"), align: "center", sortable: false },
      ];
    },
    editData(item) {
    ...
    getListData() {
      this.getDataFromApi("api/v1/departments").then((data) => {
    ...
    async deleteData(item, index) {
      if (confirm("Do you really want to delete?")) {
        ...
        const apiResponse = await ApiService.delete("api/v1/departments/" + item.id);
