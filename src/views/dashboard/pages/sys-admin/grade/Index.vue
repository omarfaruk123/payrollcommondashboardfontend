<template>
  <v-container id="grade-list" fluid tag="section">
  ...
  methods: {
    setHeaders() {
      this.headers = [
        ...
        {
          text: this.$t("grade.grade_name"),
          align: "left",
          sortable: true,
          value: "grade_name",
        },
        {
          text: this.$t("grade.status"),
          align: "left",
          sortable: false,
          value: "status",
        },
      
        { text: this.$t("grade.action"), align: "center", sortable: false },
      ];
    },
    editData(item) {
    ...
    getListData() {
      this.getDataFromApi("api/v1/grades").then((data) => {
    ...
    async deleteData(item, index) {
      if (confirm("Do you really want to delete?")) {
        ...
        const apiResponse = await ApiService.delete("api/v1/grades/" + item.id);
