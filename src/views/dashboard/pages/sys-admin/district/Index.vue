<template>
  <v-container id="district-list" fluid tag="section">
      ...
      <v-col cols="12" md="12">
        ...
        <v-card outlined class="mt-2" v-if="listp">
          ...
          <v-card-text class="mt-2">
            ...
            >
              <template slot="body">
                <tr v-for="(item, index) in districts" :key="index">
  ...
  methods: {
    setHeaders() {
      this.headers = [
        ...
        {
          text: this.$t("districtList.name"),
          align: "left",
          sortable: true,
          value: "name",
        },
        {
          text: this.$t("districtList.bn_name"),
          align: "left",
          sortable: true,
          value: "bn_name",
        },
        {
          text: this.$t("districtList.division"),
          align: "left",
          sortable: true,
          value: "division.name",
        },
    ...
    getListData() {
      this.getDataFromApi("api/district").then((data) => {
    ...
    async deleteData(item, index) {
      if (confirm("Do you really want to delete?")) {
        ...
        const apiResponse = await ApiService.delete("api/district/" + item.id);
